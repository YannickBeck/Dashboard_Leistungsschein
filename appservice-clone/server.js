...
function logRequestError(code, req, error) {
...
app.get("/api/modules", (_req, res) => {
  try {
    ...
    logRequestError("MODULES_LOAD_FAILED", _req, error);
    res.status(500).json({
...
app.get("/api/taxonomy", (_req, res) => {
  try {
    ...
    logRequestError("TAXONOMY_LOAD_FAILED", _req, error);
    res.status(500).json({
...
app.get("/api/offers", (_req, res) => {
  try {
    ...
    logRequestError("OFFERS_LOAD_FAILED", _req, error);
    res.status(500).json({
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    logRequestError("DOCX_GENERATION_FAILED", req, error);
    res.status(500).json({
